...
class _SalesScreenState extends State<SalesScreen> {
  ...
  Map<int, double> _allocatedBySale = {};
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((sale) {
        ...
        return sale.invoiceNumber.toLowerCase().contains(query) ||
            ...
            (sale.dueDate?.toLowerCase().contains(query) ?? false) ||
            (sale.notes?.toLowerCase().contains(query) ?? false) ||
            _saleTypeLabel(sale).toLowerCase().contains(query) ||
            _saleStatusLabel(sale).toLowerCase().contains(query);
      }).toList();
    }

    filtered = filtered.where(_matchesSelectedFilter).toList();
  ...
  double _allocatedAmount(Sale sale) => _allocatedBySale[sale.id] ?? 0.0;

  double _outstandingAmount(Sale sale) =>
      ((sale.total - _allocatedAmount(sale)).clamp(0.0, double.infinity) as num)
          .toDouble();

  bool _isPaid(Sale sale) {
    if (sale.status == 'PAID') return true;
    if (sale.status == 'VOID') return false;
    return _outstandingAmount(sale) <= 0.01;
  }
  ...
  bool _matchesSelectedFilter(Sale sale) {
    return switch (_selectedFilter) {
      ...
      'Outstanding' =>
        sale.status == 'NORMAL' && _outstandingAmount(sale) > 0.01,
      'Paid' => _isPaid(sale),
  ...
  String _saleTypeLabel(Sale sale) =>
      sale.saleType.toUpperCase() == 'CASH' ? 'Cash Sale' : 'Credit Sale';

  Color _saleTypeColor(Sale sale) =>
      sale.saleType.toUpperCase() == 'CASH' ? Colors.green : Colors.blue;

  String _saleStatusLabel(Sale sale) {
    if (sale.status == 'VOID') return 'Void';
    if (_isPaid(sale)) return 'Paid';
    return 'Outstanding';
  }
  ...
  Widget _buildSearchAndFilters() {
    return Container(
      ...
      child: ResponsiveFilterPanel(
        ...
        filters: [
          DropdownButtonFormField<String>(
            ...
            items: const [
              ...
              DropdownMenuItem(value: 'Cash Sales', child: Text('Cash Sales')),
              DropdownMenuItem(
                  value: 'Credit Sales', child: Text('Credit Sales')),
              DropdownMenuItem(
        ...
        hasActiveFilters: _selectedFilter != 'All' || _searchQuery.isNotEmpty,
        onClearFilters: () {
          setState(() {
            _selectedFilter = 'All';
            _searchQuery = '';
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              children: [
                                _buildPill(
                                    _saleTypeLabel(sale), _saleTypeColor(sale)),
                                _buildPill(
                                  _saleStatusLabel(sale),
                                  isVoid
                                      ? Colors.red
                                      : _isPaid(sale)
                                          ? Colors.green
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: Text(sale.dueDate ?? '-')),
                        Expanded(
                          child: Align(
  ...
  void _showSaleDetail(Sale sale) {
    ...
    // Load sale items then show dialog
    _db.getSaleItems(sale.id).then((rawItems) {
      final items = rawItems.cast<SaleItem>();
      if (!mounted) return;
        ...
        builder: (context) => ResponsiveDialog(
              ...
              children: [
                Container(
                  ...
                  child: Icon(
                    Icons.receipt_long,
                    color:
                        isVoid ? Colors.red.shade700 : Colors.purple.shade700,
                    size: isMobile
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          '${_saleTypeLabel(sale)} • ${_saleStatusLabel(sale)}',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: AppTypography.mobileLabel,
                            fontWeight: FontWeight.bold,
                ...
                IconButton(
                  icon: const Icon(Icons.close,
                      color: Colors.white, size: AppIconSizes.listItem),
            ...
            children: [
              _buildDetailSection('Sale Information', Icons.info, [
                _detailRow(
                  ...
                  'Type',
                  _saleTypeLabel(sale),
                  valueColor: _saleTypeColor(sale),
                ),
                _detailRow(Icons.calendar_today, 'Date', sale.date),
                _detailRow(
                    Icons.event_available, 'Payment Due', sale.dueDate ?? '-'),
                if (customer != null)
          ...
          actions: [
            ...
            if (!isVoid)
              TextButton.icon(
                ...
                icon: const Icon(Icons.cancel,
                    color: Colors.red, size: AppIconSizes.button),
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                minimumSize: const Size(
                    AppSpacing.minTouchTarget, AppSpacing.minButtonHeight),
              ),
  ...
  Widget _buildItemsSection(List<SaleItem> items) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            Icon(Icons.shopping_cart,
                size: AppIconSizes.button, color: Colors.purple.shade700),
            const SizedBox(width: 8),
            Text(
              'Items',
              style: TextStyle(
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              // Item rows — use FutureBuilder to resolve product names
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(
                      width: 190,
                      child: Text(
                        '£${items.fold(0.0, (s, i) => s + i.total).toStringAsFixed(2)}',
                        textAlign: TextAlign.right,
                        style: TextStyle(
...
/// Resolves the product name for a sale item and renders one table row.
...
class _SaleItemRowState extends State<_SaleItemRow> {
  ...
  void initState() {
    super.initState();
    AppDatabase.instance.getProductById(widget.item.productId).then((p) {
      if (mounted) setState(() => _productName = p?.name ?? 'Unknown');
    });
  }
