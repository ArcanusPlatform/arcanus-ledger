...
/// Receipt detail dialog for viewing receipt information and allocations
///
/// This read-only dialog displays:
/// - Receipt header (customer name, date, amount, type, reference)
          ...
          Row(
            children: [
              Expanded(
                child: _buildInfoItem(
                  'Type',
                  _receiptTypeLabel(_paymentData!['receiptType'] as String),
                  Icons.payment,
                ),
              ),
              const SizedBox(width: 16),
...
/// Receipt form dialog for creating new customer receipts
///
/// This dialog allows users to:
/// - Select a customer
/// - Enter receipt details (date, amount, reference)
  ...
  Future<void> _savePayment() async {
    ...
    try {
      ...
      final payment = PaymentsCompanion(
        ...
        receiptType: const drift.Value('CREDIT_RECEIPT'),
        paymentMethod: const drift.Value('CREDIT_RECEIPT'),
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'e.g., customer receipt note',
              ),
            ),
...
int _comparePeopleByName(PeopleData a, PeopleData b) =>
...
class _CustomerReceiptsScreenState extends State<CustomerReceiptsScreen> {
  ...
  String _selectedReceiptTypeFilter = 'All';
  ...
  void _applyFilters() {
    var filtered = _payments.where((payment) {
      ...
      final receiptType =
          _receiptTypeLabel(payment['receiptType'] as String).toLowerCase();

      return customerName.contains(query) ||
          reference.contains(query) ||
          receiptType.contains(query);
    }).toList();

    if (_selectedReceiptTypeFilter != 'All') {
      filtered = filtered
          .where((payment) =>
              payment['receiptType'] as String == _selectedReceiptTypeFilter)
          .toList();
    }
  ...
  Future<void> _deletePayment(Map<String, dynamic> payment) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete this receipt of £${(payment['amount'] as double).toStringAsFixed(2)} from ${payment['customerName']}?\n\nThis will deactivate all allocations associated with this receipt.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to delete receipt: $e')));
      }
    }
  }
  ...
  void _clearFilters() {
    setState(() {
      ...
      _selectedReceiptTypeFilter = 'All';
    });
    _applyFilters();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          Container(
            ...
            child: ResponsiveFilterPanel(
              searchField: TextField(
                decoration: InputDecoration(
                  hintText: 'Search by customer, reference, or receipt type...',
              ...
              filters: [
                DropdownButtonFormField<String>(
                  initialValue: _selectedReceiptTypeFilter,
                  decoration: InputDecoration(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(
                      value: 'CASH_SALE',
                      child: Text('Cash Sale Receipts'),
                    ),
                    DropdownMenuItem(
                      value: 'CREDIT_RECEIPT',
                      child: Text('Credit Receipts'),
                    ),
                    DropdownMenuItem(
                      value: 'FINANCE_SETTLEMENT',
                      child: Text('Ledger Settlements'),
                    ),
                  ],
                  onChanged: (value) {
                    if (value != null) {
                      setState(() => _selectedReceiptTypeFilter = value);
                      _applyFilters();
                    }
                  },
                ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(isMobile ? 'Receipt' : 'New Receipt'),
  ...
  Widget _buildEmptyState() {
    return ResponsiveEmptyState(
      icon: Icons.payment,
      title: _searchQuery.isEmpty ? 'No Receipts Yet' : 'No Receipts Found',
      subtitle: _searchQuery.isEmpty
          ? 'Start recording customer receipts by tapping the button below'
                  ...
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(
                          _receiptTypeLabel(payment['receiptType'] as String),
                          style: AppTypography.getBodyTextStyle(context),
                        ),
                      ],
                    ),
