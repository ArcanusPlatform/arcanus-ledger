...
class _SaleFormDialogState extends State<SaleFormDialog> {
  ...
  void _syncDueDate({bool force = false}) {
    ...
    final dueDate = _invoiceDateOrToday().add(
      Duration(days: customer.paymentTermsDays),
    );
    _dueDateController.text = _isoDate(dueDate);
  }

  void _handleCustomerChanged(PeopleData? customer) {
    setState(() {
  ...
  Future<void> _pickDueDate() async {
    final selected = await showDatePicker(
      ...
      initialDate: _parseIsoDate(_dueDateController.text) ??
          _invoiceDateOrToday().add(
            Duration(days: _selectedCustomer?.paymentTermsDays ?? 0),
          ),
      firstDate: DateTime(2000),
      lastDate: DateTime(2100),
    );
  ...
  Future<void> _saveSale() async {
    ...
    final saleCompanion = SalesCompanion(
      ...
      dueDate: drift.Value(_dueDateController.text.trim()),
      saleType: drift.Value(_paidInCash ? 'CASH' : 'CREDIT'),
      total: drift.Value(_totalAmount),
      status: drift.Value(_paidInCash ? 'PAID' : 'NORMAL'),
      notes: drift.Value(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_selectedCustomer != null) ...[
                        ...
                        const SizedBox(
                          height: AppSpacing.mobileFormFieldSpacing,
                        ),
                      ],
                      ...
                      Container(
                        ...
                        child: SwitchListTile(
                          ...
                          subtitle: const Text(
                            'Paid now — creates matching receipt',
                            style: TextStyle(
                              fontSize: 12,
                            ),
                          ),
...
class SaleLineItemDialog extends StatefulWidget {
  final List<Product> products;
  final SaleLineItem? existingItem;
  final Function(SaleLineItem) onAdd;

  const SaleLineItemDialog({
    super.key,
  ...
  @override
  State<SaleLineItemDialog> createState() => _SaleLineItemDialogState();
}

class _SaleLineItemDialogState extends State<SaleLineItemDialog> {
  final _formKey = GlobalKey<FormState>();
